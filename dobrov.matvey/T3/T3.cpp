...
int main(int argc, char* argv[]) {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (cmd == "MAX") handleExtremum(iss, polygons, true);
        if (cmd == "MIN") handleExtremum(iss, polygons, false);
        if (cmd == "COUNT") handleCount(iss, polygons);
        if (cmd == "ECHO") handleEcho(iss, polygons);
        if (cmd == "RIGHTSHAPES") handleRightShapes(polygons);
    }
}
...
void handleEcho(std::istringstream& iss, std::vector<Polygon>& polygons) {
    ...
    if (!(iss >> target) || !hasNoMoreArguments(iss)) {
        for (auto print : target.points_) {
            std::cout << print.x_ << "\t" << print.y_ << "\n";
        }
        std::cout << "<INVALID COMMAND>\n";
        return;
    }
